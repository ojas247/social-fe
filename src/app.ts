...
import type {
 ...
 // Configurations
 import type { AppConfig } from "./configs/types.js";
...
const configs = {
 ...
 export class A2UILayoutEditor extends SignalWatcher(LitElement) {
  ...
  const appKey: keyof typeof configs =
    appParam && appParam in configs ? (appParam as keyof typeof configs) : "restaurant";
  this.config = configs[appKey];
  ...
  console.log("#maybeRenderForm Called")
  ...
  if (this.requesting) {
      ...
      if (this.config.loadingText) {
        ...
        text =
          ...
          this.config.loadingText[0] ??
          text;
        } else {
        text = this.config.loadingText ?? text;
        }
      }
    ...
    return html`<section id="surfaces">
      ...
      ([surfaceId, surface]) => {
        return html`<a2ui-surface
          ...
          const srcContext = (action as any)?.["context"];
          if (srcContext) {
              for (const item of srcContext) {
                ...
                const sourceComponent = evt.detail.sourceComponent;
                if (!sourceComponent) {
                  continue;
                }
  ...
  async #sendAndProcessMessage(request: v0_8.Types.A2UIClientEventMessage) {
    console.log("#sendAndProcessMessage Called")
