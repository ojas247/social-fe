...
export class A2uiMessageProcessor implements MessageProcessor {
  ...
  private convertKeyValueArrayToMap(arr: DataArray): DataMap {
    ...
    for (const item of arr) {
      ...
      // Index access may be undefined at type level; cast to DataValue since
      // we only proceed when a value key was found.
      let value = item[valueKey] as DataValue;
  ...
  private setDataByPath(root: DataMap, path: string, value: DataValue): void {
    ...
    if (
      ...
      if (value.length === 1 && isObject(value[0]) && value[0].key === ".") {
        ...
        if (valueKey) {
          // Extract the primitive value. At the type level this can be
          // DataValue | undefined, but here we treat missing values as
          // malformed and still follow the existing behavior.
